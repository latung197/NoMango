...
export function ProductColorMappingTab() {
    ...
    if (view === 'edit' && editingMapping) {
      ...
      setMappings(prev => prev.map(m => 
        m.id === editingMapping.id 
          ? { ...m, ...formData, mappingCode, status: formData.status ? 'active' : 'inactive' }
          : m
      ));
    } else {
      // Add new mapping
                ...
                <div className="p-4 bg-slate-50 rounded-lg">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                    ...
                    <div>
                      ...
                      <div className="text-slate-600 font-mono font-medium text-blue-600">{formData.fullCode}</div>
                    </div>
                  </div>
                </div>
              </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Product-Color Mapping</h3>
          <p className="text-sm text-slate-600">ထုတ်ကုန်နှင့်အရောင် ပေါင်းစပ်မှု</p>
        </div>
