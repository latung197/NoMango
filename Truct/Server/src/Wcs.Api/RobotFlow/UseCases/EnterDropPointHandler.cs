...
public sealed class EnterDropPointHandler(ILogger<EnterDropPointHandler> logger, IRcsClient rcsClient, IEventPublisher publisher, ITaskStateStore store, IOpcUaClient opcUaClient, IStationHandshakeService stationHandshakeService)
{
  private readonly ILogger<EnterDropPointHandler> _logger = logger;
  ...
  public async Task Handle(FlowTask flowTask, CancellationToken ct)
  {
      ...
      if (station.HasWarehouseDoor) {
        await _stationHandshakeService.Inbound_EnterDropPointAsync(ct);
      } else {
      ...
      await _rcsClient.ContinueTaskOrThrowAsync(_store, flowTask, new ContinueTaskRequest(
        ...
        RcsPositions.Berth(flowTask.ToStation.MainPoint)
      ), ct);

      flowTask.UpdateStep(FlowStep.EnterDropPoint);
