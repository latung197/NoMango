...
import { getProducts, searchFilters } from "@/services/product.service";
...
export default function SearchText() {
    ...
    useEffect(() => {
        const fetchFilters = async () => {
            const data = await searchFilters({ name });
    ...
    useEffect(() => {
        ...
        fetchProducts();
    }, [name, page, selectedCategories, selectedLeadTimes]);
    ...
    return (
        <div className="p-4">
            <Breadcrumb
                name={`${t("search.search_result")} "${name}"`}
